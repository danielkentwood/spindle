...
class GraphStore:
    ...
    def get_duplicate_clusters(self) -> List[List[str]]:
        ...
        same_as_edges = self.query_by_pattern(predicate="SAME_AS")
    ...
    def get_canonical_entity(self, name: str) -> Optional[str]:
        ...
        same_as_edges = self.query_by_pattern(subject=name, predicate="SAME_AS")
        ...
        reverse_edges = self.query_by_pattern(predicate="SAME_AS", obj=name)
    ...
    def query_with_resolution(
        ...
        if not resolve_duplicates:
            return self.query_by_pattern(subject, predicate, obj)
        ...
        if subject is not None:
            canonical = self.get_canonical_entity(subject)
            if canonical:
                resolved_subject = canonical
        
        if obj is not None:
            canonical = self.get_canonical_entity(obj)
            if canonical:
                resolved_obj = canonical
        
        # Query with resolved entities
        results = self.query_by_pattern(resolved_subject, predicate, resolved_obj)
        ...
        for edge in results:
            ...
            edge_subj_canonical = self.get_canonical_entity(edge['subject']) or edge['subject']
            edge_obj_canonical = self.get_canonical_entity(edge['object']) or edge['object']
